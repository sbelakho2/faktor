...
#   FAKTOR_PLUS
# Retired product-name tokens are owned by the static source scan
# (`tests/static-authority`, `faktor-tests-static-authority`), which
# forbids the product name everywhere except the historical attribution
# under ui/LICENSES/; this script owns the packaging wordmarks.
#
# Modes:
...
#       (any value carrying faktor-plus is a hit)
#     - Clap command metadata and Cargo.toml package/[[bin]] name fields
#       (source-level `name = "faktor-plus"` hits; a legacy Clap name or
#       binary name is a hit)
#     - default daemon data-dir constants in crates/cli
#       (`default_value = "...faktor-plus..."` hits; the data dir itself
#       is `~/.faktor`)
...
#   * paths under vendor/ and third-party/ — external sources; entries that
#     do not exist are tolerated (no compat/ or vendored UI tree exists)
...
ALLOWLIST_FRAGMENTS=(
  '/vendor/'
  '/third-party/'
  # The docs-drift guard must spell the identifiers it scans for, the same
  # self-reference class as this script.
...
while IFS= read -r file; do
  ...
  if grep -a -n -H -i -F -e 'FAKTOR_PLUS' -- "$file" 2>/dev/null; then
    hits=$((hits + 1))
  fi
done < <(collect_files)
  ...
  while IFS= read -r file; do
    ...
    case "$file" in
      *.rs | *.toml | *.gradle.kts | *.kt)
        ...
        if grep -a -n -H -i -E 'name[[:space:]]*=[[:space:]]*"faktor-plus' \
    ...
    case "$file" in
      */package.json)
        ...
        # publisher must carry no legacy wordmark form.
        if grep -a -n -H -i -E \
            '"(name|displayName|publisher)"[[:space:]]*:[[:space:]]*"[^"]*faktor-plus' \
            -- "$file" 2>/dev/null; then
          meta_hits=$((meta_hits + 1))
        fi
        ;;
    ...
    case "$file" in
      */crates/cli/*.rs)
        ...
        if grep -a -n -H -i -E \
            'default_value[[:space:]]*=[[:space:]]*"[^"]*faktor-plus' \
            -- "$file" 2>/dev/null; then
          meta_hits=$((meta_hits + 1))
        fi
        ;;
